    ...
    public class BudgetRulesController : BaseController
    {
        ...
        public async Task<IActionResult> CreateBudgetRule([FromBody] BudgetRules budgetRule)
        {
            try
            {
                ...
                // Validate budget amounts add up to total
                var totalBudgeted = budgetRule.Savings + budgetRule.Needs + budgetRule.Wants;
                var difference = Math.Abs(budgetRule.TotalAmount - totalBudgeted);
                
                if (difference > 0) // No tolerance needed for integers
                {
                    return JsonError($"Budget amounts (₱{totalBudgeted}) must equal total income (₱{budgetRule.TotalAmount}).");
                }
